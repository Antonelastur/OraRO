# Materiale restante, clasa a VII-a

Toate cele 5 unități sunt construite structural (119 lecții, sursate direct
din proiectarea pe unități 2024-2025, pagini verificate la sursă). 26 de
lecții au material (schemă, infografic sau joc). Rămân 93, listate mai jos pe
unitate, cu tipul de resursă orientativ.

## Recapitulare inițială

Ambele lecții sunt activități administrative de start de an, fără pagină de
manual — rămân fără material.

## Unitatea I: Călătorie în necunoscut (16 lecții)

| Lecție | Titlu | Tip orientativ |
|---|---|---|
| lectia-1 | Cum e lumea de Veronica D. Niculescu | prezentare |
| lectia-5 | Textul nonliterar, textul discontinuu. Anunțul | infografic |
| lectia-7 | Contacte culturale | infografic |
| lectia-15 | Categoriile semantice. Confuziile paronimice | schemă |
| lectia-16 | Îmbinări libere de cuvinte și locuțiuni | schemă |
| lectia-22 | Evaluare, probă scrisă | test (Google Forms) |

Gata, integrate în cod: fișele lecțiilor 2, 3, 4, 6, 18, 19 (`scripts/build_clasa7_fise.py`) și jocurile-quiz ale lecțiilor 8, 14, 17. Lecția 9 rămâne intenționat fără material (manualul are joc de rol). Rămân: 2 infografice (5, 7), 2 scheme (15, 16), prezentarea (1) și testul (22).

## Unitatea II: Aproape de ceilalți (20 lecții)

| Lecție | Titlu | Tip orientativ |
|---|---|---|
| lectia-1 | Proiect de grup. Valorile mele, valorile comunității (anunțare) | fișă (cerințe și criterii) |
| lectia-2 | Popa Tanda de Ioan Slavici | prezentare |
| lectia-3 | Structura textului narativ | fișă |
| lectia-4 | Mijloace de construcție a personajului. Repetiția | fișă |
| lectia-5 | Conflictul. Complexitatea personajului | fișă |
| lectia-6 | Semnificațiile textului | fișă |
| lectia-7 | Text auxiliar. Inocenții de Ioana Pârvulescu | fișă |
| lectia-8 | Prezentarea proiectului Valorile mele, valorile comunității | fișă (grilă de evaluare) |
| lectia-9 | Despre limba și cultura țărilor vecine (I) | infografic |
| lectia-10 | Despre limba și cultura țărilor vecine (II) | infografic |
| lectia-11 | Fapte și opinii. Textul explicativ (I) | fișă |
| lectia-12 | Fapte și opinii. Textul explicativ (II) | joc |
| lectia-14 | Verbul. Tipuri de verbe. Locuțiunea verbală (II) | fișă |
| lectia-15 | Modurile și timpurile verbului | schemă |
| lectia-16 | Forme verbale nepersonale: infinitivul și participiul | schemă |
| lectia-17 | Forme verbale nepersonale: gerunziul și supinul | schemă |
| lectia-19 | Posibilități combinatorii ale verbului. Circumstanțialul de cauză (II) | fișă |
| lectia-21 | Caracterizarea personajului (I), pregătire | fișă |
| lectia-22 | Caracterizarea personajului (II), redactare | fișă |
| lectia-25 | Evaluare, probă scrisă | test (Google Forms) |

## Unitatea III: Harta sentimentelor (20 lecții)

| Lecție | Titlu | Tip orientativ |
|---|---|---|
| lectia-1 | Lacul de Mihai Eminescu. Structura textului poetic | prezentare |
| lectia-2 | Textul liric. Exprimarea emoțiilor și a sentimentelor | fișă |
| lectia-3 | Limbajul figurat | schemă |
| lectia-4 | Semnificațiile textului | fișă |
| lectia-5 | Am legat... de Marin Sorescu | prezentare |
| lectia-7 | Semnificațiile textului | fișă |
| lectia-8 | Text auxiliar. Iarna de Mircea Cărtărescu | fișă |
| lectia-9 | Tradiții și obiceiuri legate de sentimente și emoții (I) | infografic |
| lectia-10 | Tradiții și obiceiuri legate de sentimente și emoții (II) | infografic |
| lectia-11 | Atitudini comunicative | joc |
| lectia-12 | Textul descriptiv (I) | fișă |
| lectia-13 | Textul descriptiv (II) | joc |
| lectia-14 | Substantivul | schemă |
| lectia-16 | Posibilități combinatorii ale substantivului | schemă |
| lectia-18 | Posibilități combinatorii ale adjectivului | schemă |
| lectia-19 | Textul descriptiv (III), pregătire | fișă |
| lectia-20 | Textul descriptiv (IV), redactare | fișă |
| lectia-21 | Comentarea unor pasaje. Descrierea unei emoții (I) | fișă |
| lectia-22 | Comentarea unor pasaje. Descrierea unei emoții (II) | fișă |
| lectia-25 | Evaluare, probă scrisă | test (Google Forms) |

## Unitatea IV: Cu noroc, fără noroc (19 lecții)

| Lecție | Titlu | Tip orientativ |
|---|---|---|
| lectia-1 | Două loturi de I.L. Caragiale | prezentare |
| lectia-2 | Spațiul, timpul, acțiunea | fișă |
| lectia-3 | Naratorul și personajele | fișă |
| lectia-4 | Semnificațiile textului | fișă |
| lectia-5 | Textul multimodal (actualizare). Enciclopedia (I) | infografic |
| lectia-7 | Text auxiliar. Cerșetorul și regele | fișă |
| lectia-8 | Elemente de mitologie românească: Baba-Dochia și mărțișorul | infografic |
| lectia-9 | Structura textului narativ oral | joc |
| lectia-10 | Strategii de concepere și de comprehensiune a textului oral | joc |
| lectia-12 | Pronumele și adjectivul pronominal posesiv | schemă |
| lectia-13 | Pronumele și adjectivul pronominal demonstrativ | schemă |
| lectia-14 | Pronumele și adjectivul pronominal nehotărât | schemă |
| lectia-15 | Pronumele și adjectivul pronominal interogativ | schemă |
| lectia-16 | Pronumele și adjectivul pronominal relativ | schemă |
| lectia-17 | Pronumele și adjectivul pronominal negativ | schemă |
| lectia-18 | Adjectivul pronominal de întărire | schemă |
| lectia-20 | Structuri textuale: narativ, explicativ, descriptiv, dialogat (I) | fișă |
| lectia-21 | Structuri textuale: narativ, explicativ, descriptiv, dialogat (II) | fișă |
| lectia-24 | Evaluare, probă scrisă | test (Google Forms) |

## Unitatea V: Lumea de pe scenă (16 lecții)

| Lecție | Titlu | Tip orientativ |
|---|---|---|
| lectia-1 | Proiect de grup. De la text la spectacol (anunțare) | fișă (cerințe și criterii) |
| lectia-2 | Vrem să vă dăruim câte o fereastră de Matei Vișniec | prezentare |
| lectia-3 | Textul dramatic. Structura, rolul indicațiilor scenice, autorul | schemă |
| lectia-4 | Arta spectacolului. Personajul dramatic | fișă |
| lectia-5 | Rolul dialogului. Dialogul în textul scris și în spectacol | fișă |
| lectia-6 | Semnificațiile textului | fișă |
| lectia-7 | Dialogul în textul nonliterar. Interviul (I) | fișă |
| lectia-8 | Dialogul în textul nonliterar. Interviul (II) | joc |
| lectia-9 | Text auxiliar. Visul unei nopți de vară de Shakespeare | fișă |
| lectia-10 | Prezentarea proiectului De la text la spectacol (I) | fișă (grilă de evaluare) |
| lectia-11 | Evaluarea proiectului De la text la spectacol (II) | fișă (grilă de evaluare) |
| lectia-12 | Textul dialogat. Tracul comunicativ | joc |
| lectia-13 | Strategii de ascultare activă | joc |
| lectia-17 | Tipare textuale de structurare a ideilor. Cererea (I) | fișă |
| lectia-18 | Tipare textuale de structurare a ideilor. Cererea (II) | fișă |
| lectia-21 | Evaluare, probă scrisă | test (Google Forms) |

## Total

**119 lecții** construite structural (recapitulare inițială + 5 unități).
**26 au material** (9 schemă, 3 infografic, 14 joc). **93 rămân cu
`materiale` null** — listate mai sus, tip orientativ, de completat într-o
sesiune dedicată, cu aceeași convenție ca la clasele a V-a și a VI-a: schemă/
infografic/joc/fișă pot fi generate direct în cod, prezentare și test au
nevoie de o unealtă externă (NotebookLM/Canva, respectiv Google Forms).

Clasa a VII-a rămâne neînregistrată în `clase.js`.
